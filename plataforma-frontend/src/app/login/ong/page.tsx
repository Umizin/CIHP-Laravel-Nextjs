...
export default function LoginOng() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost/api/login/ong", {
        method: "POST",
      ...
      <div className="flex flex-col justify-center items-center w-full md:w-1/2 px-6 py-10 bg-white">
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-md flex flex-col gap-4">
          ...
          <p className="text-sm text-center text-gray-600 mt-2">
            Ainda não tem conta?{" "}
            <Link href="/cadastro/ong" className="text-yellow-500 hover:underline">
              Cadastre-se aqui!
            </Link>
          </p>
        </form>
