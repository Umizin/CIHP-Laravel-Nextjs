...
import { useState, useEffect } from "react";
import { User, Settings, HelpCircle, LogOut, Folder, Menu, PlusCircle, Briefcase, Users } from "lucide-react";
import Link from "next/link";
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    if (data) {
      try {
        ...
        setUserType(parsed.user?.type || null);
      } catch {
        setUserType(null);
      }
    }
  ...
  // Fecha automaticamente o menu no desktop
  useEffect(() => {
    const handleResize = () => {
  ...
  const getMenuLinks = () => {
    switch (userType) {
      case "ong":
        return [
          { href: "/perfil/ong", icon: User, label: "Perfil" },
          { href: "/trabalhos/ONGS/vagas", icon: Folder, label: "Minhas vagas" },
          { href: "/trabalhos/ONGS/publicar", icon: PlusCircle, label: "Criar vaga" },
        ];
      case "empresa":
        return [
          { href: "/perfil/empresa", icon: User, label: "Perfil" },
          { href: "/projetos", icon: Briefcase, label: "Projetos" },
          { href: "/parcerias", icon: Users, label: "Parcerias" },
        ];
      default: // voluntário
        return [
          { href: "/perfil/voluntario", icon: User, label: "Perfil" },
          { href: "/trabalhos/voluntarios/minhas-inscricoes", icon: Folder, label: "Meus trabalhos" },
        ];
    }
  };
    ...
    <>
      ...
      >
        <div className="flex flex-col h-full overflow-hidden">
          ...
          {/* Navegação */}
          <nav className="mt-4 flex flex-col flex-grow space-y-2 px-1 overflow-y-auto">
